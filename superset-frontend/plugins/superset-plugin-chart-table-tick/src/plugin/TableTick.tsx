...
import { DataColumnMeta, TableCheckboxTransformedProps } from './types';
import DataTable, {
  DataTableProps,
  SearchInputProps,
...
export default function TableTick<D extends DataRecord = DataRecord>(
  props: TableCheckboxTransformedProps<D> & {
  ...
  const getColumnConfigs = useCallback(
    (column: DataColumnMeta, i: number): ColumnWithLooseAccessor<D> => {
      ...
      const { useTick, tickText, iconHeight } = config
            ...
            <StyledCell {...cellProps}>
              {truncateLongCells ? (
                <div
                  ...
                  style={columnWidth ? { width: columnWidth } : undefined}
                >
                  {useTick ? TickIcon(text, tickText, iconHeight) : text}
                </div>
              ) : (
                useTick ? TickIcon(text, tickText, iconHeight) : text
              )}
